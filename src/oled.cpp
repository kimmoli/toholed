...
void drawAnalogClock(int hours, int minutes, char *screenBuffer)
{
    ...
    minuteHand.angle = ((2 * M_PI / 60) * minutes) - M_PI_2;
    hourHand.angle = ((2 * M_PI / 12) * (hours + ((float)minutes/60))) - M_PI_2;
...
void drawHand(const analogHand hand, int color, char *screenBuffer)
{
    ...
    for (int i = -hand.base_radius; i < hand.base_radius; i++)
    {
        if (i != 0)
        {
            hand_base_x = i * cos (hand.angle + M_PI_2) + clock_x;
            hand_base_y = i * sin (hand.angle + M_PI_2) + clock_y;
